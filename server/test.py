...
def top_tracks(sp):
    ...
    for idx, item in enumerate(results['items']):
        track = item['name']
        track_data[item['name']] = [characteristics(sp, item['uri'])]
...
'''
def find_artist_top_tracks(sp, artist_id):
    result = sp.artist_top_tracks(artist_id)
    tracks = {}
    for track in result['tracks']: 
        tracks[track['name']] = characteristics(sp, track['uri'])
